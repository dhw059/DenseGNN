...
class SetEdgeIndicesReverse(GraphPreProcessorBase):
    ...
    def __init__(self, *, range_indices: str = "range_indices", edge_indices_reverse: str = "edge_indices_reverse",
                 name="set_edge_indices_reverse", **kwargs):
        super().__init__(name=name, **kwargs)
        self._to_obtain.update({"range_indices": range_indices})
        self._to_assign = edge_indices_reverse
        self._config_kwargs.update({"range_indices": range_indices, "edge_indices_reverse": edge_indices_reverse})

    def call(self, *, range_indices: np.ndarray):
        if range_indices is None:
            return None
        return np.expand_dims(compute_reverse_edges_index_map(range_indices), axis=-1)
...
class SetEdgeWeightsUniform(GraphPreProcessorBase):
    ...
    def __init__(self, *, range_indices: str = "range_indices", edge_weights: str = "edge_weights",
                 value: float = 1.0, name="set_edge_weights_uniform", **kwargs):
        super().__init__(name=name, **kwargs)
        ...
        self._call_kwargs = {"value": value}
        self._to_assign = edge_weights
        self._config_kwargs.update({"range_indices": range_indices, "edge_weights": edge_weights, **self._call_kwargs})

    def call(self, *, range_indices: np.ndarray, value: float):
        if range_indices is None:
            return None
        return np.ones((len(range_indices), 1)) * value
...
class SetRange(GraphPreProcessorBase):
    ...
    def call(self, *, node_coordinates: np.ndarray, range_indices: np.ndarray,
        ...
        '''
        if range_indices is not None and not overwrite: # False
...
class SetRangePeriodic(GraphPreProcessorBase):
    ...
    def call(self, *, node_coordinates: np.ndarray, graph_lattice: np.ndarray,
        ...
        if all([range_item is not None for range_item in [range_indices, range_image]]) and not overwrite: # False
