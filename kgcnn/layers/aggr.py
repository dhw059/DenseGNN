...
class AggregateLocalEdges(GraphBaseLayer):
    ...
    def call(self, inputs, **kwargs):
        ...
        其实这就是一个典型的graph的GCN形式，聚集了中心node周围的邻居edges和nodes，即information is passed between nodes vias edges;
...
class AggregateWeightedLocalEdges(AggregateLocalEdges):
    ...
    def call(self, inputs, **kwargs):
        ...
        out = self._aggregate(nodes, edges_weighted, receive_indices)  # 考虑了自己的nodes信息
...
class AggregateLocalEdgesAttention(AggregateLocalEdges):
    ...
    def call(self, inputs, **kwargs):
        ...
        alpha = segment_softmax(attention, receive_indices) # edges,1; 
        edges_attended = edges * alpha
        out = tf.math.segment_sum(edges_attended, receive_indices) # node level
