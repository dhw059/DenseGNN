...
from kgcnn.layers.aggr import AggregateLocalEdgesAttention, AggregateLocalEdges
...
class AttentionHeadGAT(GraphBaseLayer):
    ...
    def call(self, inputs, **kwargs):
        ...
        w_n = self.lay_linear_trafo(node, **kwargs) # nodes,32;
        wn_in = self.lay_gather_in([w_n, edge_index], **kwargs)  # edges,32;
        wn_out = self.lay_gather_out([w_n, edge_index], **kwargs) # edges,32;
        if self.use_edge_features: 
            e_ij = self.lay_concat([wn_in, wn_out, edge], **kwargs) # edges,128
        else:
            e_ij = self.lay_concat([wn_in, wn_out], **kwargs)
        a_ij = self.lay_alpha(e_ij, **kwargs)  # Should be dimension (batch, None,1)
        h_i = self.lay_pool_attention([node, wn_out, a_ij, edge_index], **kwargs) # # nodes,32
...
class AttentionHeadGATV2(GraphBaseLayer):
    ...
    def __init__(self,
                ...
                # activation='kgcnn>shifted_softplus',
                 ...
                 bias_constraint=None,
                 kernel_initializer='glorot_uniform',
                 bias_initializer='zeros', cfconv_pool='segment_sum',
                 **kwargs):
        """Initialize layer."""
        ...
        self.lay_dense  = Dense(units=self.units, activation='linear', use_bias=False, **kernel_args)
        self.lay_dense3 = Dense(units=self.units, activation=activation, use_bias=self.use_bias, **kernel_args)
        self.lay_dense4 = Dense(units=self.units, activation='linear', use_bias=self.use_bias, **kernel_args)
    ...
    def call(self, inputs, **kwargs):
        ...
        w_n = self.lay_linear_trafo(node, **kwargs) # Dense
