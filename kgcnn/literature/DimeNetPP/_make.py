...
def make_model(inputs: list = None,
    ...
    bond_index_input = ks.layers.Input(**inputs[2])  # range_indices
...
def make_crystal_model(inputs: list = None,
    ...
    # Make input
    node_input = ks.layers.Input(**inputs[0])
    xyz_input = ks.layers.Input(**inputs[1]) # node_coordinates
    bond_index_input = ks.layers.Input(**inputs[2]) # range_indices
    angle_index_input = ks.layers.Input(**inputs[3]) # angle_indices
    edge_image = ks.layers.Input(**inputs[4]) # range_image
    lattice = ks.layers.Input(**inputs[5]) # graph_lattice
    ...
    # Embedding block
    rbf_emb = Dense(emb_size, use_bias=True, activation=activation,
                    kernel_initializer="kgcnn>glorot_orthogonal")(rbf) # distance emb
    n_pairs = GatherNodes()([n, edi]) # edge node emd 
    x = LazyConcatenate(axis=-1)([n_pairs, rbf_emb]) # edge emb
    x = Dense(emb_size, use_bias=True, activation=activation, kernel_initializer="kgcnn>glorot_orthogonal")(x)
    ps = DimNetOutputBlock(emb_size, out_emb_size, num_dense_output, num_targets=num_targets,
                           output_kernel_initializer=output_init)([n, x, rbf, edi])
