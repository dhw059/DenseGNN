...
def make_model(inputs: list = None,
    ...
    for i in range(0, depth):
        ...
        head_node = [i[0] for i in heads]
        head_edge = [i[1] for i in heads]
        if attention_heads_concat:
            nk = LazyConcatenate(axis=-1)(head_node) # 这里加入depth的原因？级跳？
            ep = LazyConcatenate(axis=-1)(head_edge) 
        else:
            nk = LazyAverage()(head_node)
            nk = Activation(activation=attention_args["activation"])(nk)

            ep = LazyAverage()(head_edge)
            ep = Activation(activation=attention_args["activation"])(ep)
