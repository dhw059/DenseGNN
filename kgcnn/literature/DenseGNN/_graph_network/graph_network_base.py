...
class GraphNetworkBase(GraphBaseLayer):
    ...
    def aggregate_edges_local(self, nodes, edges, edge_indices):
        ...
        return is graph level

        In the GN framework this is notaded as `ρ_{E -> V}`.  edges-->nodes
    ...
    def aggregate_edges_global(self, edges):
        ...
        if self.aggregate_edges_global_ is None: # False
            return edges

        if self.aggregate_edges_global_ in self.attention_strings:  # False
            ...
            edge_features = self.get_features(edges)
            aggregated_edges = self.lay_pool_edges_global(edge_features) # E-G
    ...
    def aggregate_nodes(self, nodes):
        ...
        if self.aggregate_nodes_ is None: # False
            return nodes
        ...
        if self.aggregate_nodes_ in self.attention_strings:  # False
            ...
            node_features = self.get_features(nodes)
            aggregated_nodes = self.lay_pool_nodes(node_features)  # graph level
    ...
    def call(self, inputs, **kwargs):
        ...
        edges_new = self.update_edges(edges, nodes_in, nodes_out, globals_) # V-E ；edges, ；为什么这里没用edges两端的nodes信息？这里的edges就是gaussianbasis
        aggregated_edges_local = self.aggregate_edges_local(
            nodes, edges_new, edge_indices
        )# nodes,  E-V  sum 

        aggregated_edges_global = self.aggregate_edges_global(edges_new) # graph,； E-G  sum

        nodes_new = self.update_nodes(aggregated_edges_local, nodes, globals_)# E-V ；很奇怪 这里没有用message aggregate后的新的nodes 
        aggregated_nodes = self.aggregate_nodes(nodes_new) # graph,  ； V-G

        globals_new = self.update_globals(
            aggregated_edges_global, aggregated_nodes, globals_
        ) #graph,  这里也是没有用到aggregated_edges_global和aggregated_nodes

        edges_new = edges_new if self.return_updated_edges else edges # F
