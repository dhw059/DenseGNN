...
class HamNaiveDynMessage(GraphBaseLayer):
    ...
    def call(self, inputs, **kwargs):
        """Forward pass.
          # Hamiltonian Engine：Hamiltonian Engine模拟了分子动力学过程，用于建模小分子的构象。
        # 它基于离散化的Hamiltonian方程，通过学习的能量和耗散函数，
        # 使分子中的隐式位置和动量相互作用。最终的位置通过真实构象进行监督，并作为输入传递给Fingerprint Generator。
        ...
        return mv_ftr, me_ftr, p_uv_ftr, q_uv_ftr
