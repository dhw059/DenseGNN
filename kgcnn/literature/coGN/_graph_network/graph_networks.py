...
class GraphNetwork(GraphNetworkBase):
    ...
    def update_edges(self, edges, nodes_in, nodes_out, globals_, **kwargs):
        ...
        if self.update_edges_input[3]: # False
            global_features = self.get_features(globals_)
            features_to_concat.append(self.gather_state([global_features, edge_features]))
        
        concat_features = self.lazy_concat(features_to_concat) #本质就是node的embbeding dimension and edges emb dimension concat
        messages = self.edge_mlp(concat_features)

        if self.edge_gate is not None:  # False
            messages = tf.RaggedTensor.from_row_splits(
                    self.edge_gate(messages.values, edge_features.values)[1],
                    messages.row_splits) # filter 上下文信息
        
        if self.residual_edge_update:  # False
            messages = self.lazy_add([edge_features, messages])  # edges feature add with the message fea, keep message fea and edges fea dimension match 

        edges_new = self.update_features(edges, messages) # 这有点是两端的nodes and edges一起更新nodes之间的edges
        
        if self.aggregate_edges_local_ in self.attention_strings:  # False
        ...
        elif self.aggregate_edges_global_ in self.attention_strings: # False
    ...
    def update_nodes(self, aggregated_edges, nodes, globals_, **kwargs):
        ...
        if self.update_nodes_input[1]:  # False
            features_to_concat.append(node_features)
        if self.update_nodes_input[2]:  # False
            global_features = self.get_features(globals_)
        ...
        if self.node_gate is not None:  # False
    ...
    def update_globals(self, aggregated_edges, aggregated_nodes, globals_, **kwargs):
        ...
        if self.update_global_input[0]:
            aggregated_edge_features = self.get_features(aggregated_edges)
            features_to_concat.append(aggregated_edge_features)
        if self.update_global_input[1]:  # True
        ...
        if self.global_gate is not None:  # False
        ...
        if self.residual_global_update:  # modify-1 set this parameter to True 
            global_features = self.get_features(globals_)
...
class GraphNetworkMultiplicityReadout(GraphNetwork):
    ...
    def __init__(self, edge_mlp, node_mlp, global_mlp,
        ...
        multiplicity_invariant_pooling_operations = ["segment_min", "min", "reduce_min",
                "segment_max", "max", "reduce_max", "mean"] # "mean"
        no_normalization_pooling_operations = ["segment_sum", "sum", "reduce_sum"]
            
        if aggregate_edges_global is not None: # T
            if aggregate_edges_global in multiplicity_invariant_pooling_operations:   # T
        ...
        if aggregate_nodes is not None:  #  True
            if aggregate_nodes in multiplicity_invariant_pooling_operations: # T
    ...
    def aggregate_edges_global(self, edges, **kwargs):
        ...
        if self.use_edge_multiplicities:  # False
    ...
    def aggregate_nodes(self, nodes, **kwargs):
        ...
        if self.use_node_multiplicities:  # F
            multiplicity = self.get_attribute(nodes, 'multiplicity')
            # multiplicity = self.get_attribute(nodes, 'coordinationNumber')
            node_features = self.get_features(nodes)
            ...
            aggregated_node_features = super().aggregate_nodes(node_features_weighted, **kwargs)  # graph level
            if self.node_multiplicity_normalization:  # True
                normalization_factor = self.get_multiplicity_normalization_factor(multiplicity)
...
class CrystalInputBlock(GraphNetworkBase):
    ...
    def update_edges(self, edges, nodes_in, nodes_out, globals_, **kwargs):
        edge_features_new = self.edge_embedding(self.get_features(edges)) # 这里仅仅只是做了distance的GaussBasisExpansion
        if self.edge_mlp:
