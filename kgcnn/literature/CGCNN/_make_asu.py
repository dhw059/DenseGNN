...
from kgcnn.layers.modules import Dense, OptionalInputEmbedding ,LazyMultiply
from kgcnn.layers.casting import ChangeTensorType
from ._gin_conv import  GINE
from kgcnn.layers.mlp import GraphMLP
from kgcnn.model.utils import update_model_kwargs
from kgcnn.layers.modules import LazyConcatenate, Dense, Dropout, OptionalInputEmbedding,LazyAdd
...
def make_model_asu(inputs=None,
    ...
    """
    n_units =  128
    edge_indices = ks.Input(**inputs['edge_indices']) # edges,2
    atomic_number = ks.Input(**inputs['atomic_number']) # nodes,
    ...
    elif in_inputs('offset'):  # True
        offset = ks.Input(**inputs['offset']) # edges,3
        edge_inputs.append(offset) # # edges,3
    ...
    if in_inputs('CrystalNNFinger'):  # True
        inp_CrystalNNFinger = ks.Input(**inputs['CrystalNNFinger'])  
        # inp_CrystalNNFinger = tf.cast(inp_CrystalNNFinger, tf.float32) # nodes, 
        node_inputs.append(inp_CrystalNNFinger)
    ...
    if in_inputs('voronoi_ridge_area'):
        ...
        edge_input = distance # edges,1

    if in_inputs('CrystalNNFinger'):
        # node_input = {'features': atomic_number, 'multiplicity': inp_multiplicity_}
        node_input = {'features': atomic_number, 'CrystalNNFinger': inp_CrystalNNFinger}
    else:
    ...
    if in_inputs('line_graph_edge_indices'):
        ...
        uenv = OptionalInputEmbedding(**input_embedding['graph'],
                                    use_embedding=len(inputs['charge']['shape']) < 1)(env_input) # graph
    ...
    # multiplicity = get_attribute(node_features, 'multiplicity')
    ed = edge_features
    ud = global_input
    n_raw = n
    ed_raw = ed
    ...
    # n = LazyConcatenate()(list_embeddings) 
    ...
    # concat readout                                    out2
    # n = PoolingNodes(**pooling_args)(n)  # node-G
    # ed = PoolingGlobalEdges(**pooling_args)(ed) # ed-G
...
class GraphNetworkConfigurator():
    ...
    def get_input_block(node_size=64, edge_size=64,
                        ...
                        oxidation_states=True, melting_point=True, density=True, mendeleev=True, molarvolume=True, vanderwaals_radius=True, 
                        average_cationic_radius=True, average_anionic_radius=True, velocity_sound=True, thermal_conductivity=True,
                        electrical_resistivity=True, rigidity_modulus=True,
                        edge_embedding_args={
