...
model_crystal_default = {
    ...
    'nblocks': 3, 'has_ff': True, 'dropout': None, 'use_set2set': True,
    'verbose': 10,
    'output_embedding': 'graph', "gin_mlp":  {}, "pooling_args":{},
    'output_mlp': {"use_bias": [True, True, True], "units": [32, 16, 1],
                   "activation": ['kgcnn>softplus2', 'kgcnn>softplus2', 'linear']}
}
...
def make_crystal_model(inputs: list = None,
    ...
    """
    # Make input
    node_input = ks.layers.Input(**inputs[0]) # nodes,
    xyz_input = ks.layers.Input(**inputs[1]) # nodes,3
    edge_index_input = ks.layers.Input(**inputs[2]) # edges,2
    env_input = ks.Input(**inputs[3]) # charge,graph;
    edge_image = ks.layers.Input(**inputs[4]) # edges,3
    lattice = ks.layers.Input(**inputs[5])# graph
    ...
    if expand_distance:
        ed = GaussBasisLayer(**gauss_args)(ed) # edges,64

    # Model
    vp = n #nodes,
    ep = ed # edges,
    up = uenv # graph 
    vp = GraphMLP(**node_ff_args)(vp)# nodes
    ep = GraphMLP(**edge_ff_args)(ep) # edges
    # vp = MLP(**node_ff_args)(vp)# nodes
    # ep = MLP(**edge_ff_args)(ep) # edges
    up = MLP(**state_ff_args)(up) # graph
    vp2 = vp
    ep2 = ep
    up2 = up

    list_embeddings_n = [vp2]
    list_embeddings_e = [ep2]
    list_embeddings_u = [up2]
    for i in range(0, nblocks):
        if has_ff and i > 0:  #T
            vp2 = GraphMLP(**gin_mlp)(n)
            ep2 = GraphMLP(**gin_mlp)(ed)
            up2 = GraphMLP(**gin_mlp)(ud)
        # MEGnetBlock
        vp2,ep2,up2= MEGnetBlock(**meg_block_args)(
            [vp2, ep2, edi, up2])
        
         # be choosed
        list_embeddings_n.append(vp2)
        list_embeddings_e.append(ep2)
        list_embeddings_u.append(up2)
    ...
    # raw megnet message and aggregate part
    # for i in range(0, nblocks):
    #     if has_ff and i > 0:  #T
    #         vp2 = GraphMLP(**node_ff_args)(vp)
    #         ep2 = GraphMLP(**edge_ff_args)(ep)
    ...
    # ep = ks.layers.Flatten()(ep) # graph
    # vp = ks.layers.Flatten()(vp) # graph
    # final_vec = ks.layers.Concatenate(axis=-1)([vp,ep,up]) # graph


    if dropout is not None: #False
        final_vec = ks.layers.Dropout(dropout, name='dropout_final')(final_vec)

    # Only graph embedding for Megnet
    if output_embedding != "graph":  # False
        raise ValueError("Unsupported output embedding for mode `Megnet`.")
